...
heston_model = LogHestonProblem(μ, κ, θ, σ, ρ, log.([50.0, v0]), tspan, seed=rand(UInt64))

function reseed_log_heston(prob, i, repeat)
    LogHestonProblem(μ, κ, θ, σ, ρ, log.([50.0, v0]), tspan, seed=rand(UInt64))
end
