...
function compute_linear_observations(ensemble, H, R)
    L = (cholesky(R).L)
    obs_dim = size(H)[1]
    ...
    Threads.@threads for i in eachindex(ensemble)
        ...
        observation_array[i] = (Ref(H) .* ensemble[i].u) .+ (Ref(L) .* randn.(fill(obs_dim, n_obs)))
    end
    return observation_array
end

sparsity_amount = 0.2
H = rand(observation_dimension, state_dimension)
H .*= (rand(observation_dimension, state_dimension) .> sparsity_amount)
R = rand(observation_dimension, observation_dimension)
R *= R'
R *= 1.0^2
print("H = $H")
print("R = $R")
observations = compute_linear_observations(sim, H, R);
...
function save_sim_obs_to_file(path, sim, obs, H, R)
    mkpath(path)
    ...
    CSV.write(path * "H.csv", Tables.table(H), writeheader=false)
    CSV.write(path * "R.csv", Tables.table(R), writeheader=false)
    return dataframe_joined
end

current_datetime_fs_string = Dates.format(now(), "yyyy-mm-dd_HH-MM-SS")
save_path = "./data/L96_SYSTEM_$(current_datetime_fs_string)_$(state_dimension)_DIM_STATE_$(size(H)[1])_DIM_OBS_WITH_$(sparsity_amount*100)PERCENT_SPARSITY/";
