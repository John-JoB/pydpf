...
class LorenzMeasurement(pydpf.Module):
    def __init__(self, state_dim:int, obs_dim:int,  device: Union[str, torch.device] = "cpu", generator: torch.Generator = torch.default_generator, obs_matrix = None, measurement_cov = None):
        ...
        if obs_matrix is None:
            self.observation_matrix = torch.nn.Parameter(torch.ones(size=(obs_dim, state_dim), device=device) * torch.rand(size=(obs_dim,state_dim), device = device, generator=generator)/10)
        else:
            self.observation_matrix = obs_matrix
        self.observation_dist = pydpf.distributions.LinearGaussian(self.observation_matrix, torch.zeros(size=(obs_dim,), device=device), root_measurement_cov, diagonal_cov=False, generator=generator)
    ...
    def d_score(self, state:Tensor, observation:Tensor, time, prev_time = None) -> Tensor:
        return -self.observation_dist.d_log_density(observation.unsqueeze(1), state) @ self.observation_matrix
...
class LorenzDynamic(pydpf.Module):
    ...
    def d_log_density(self, state:Tensor, prev_state:Tensor, time, prev_time):
        ...
        root_gap = torch.sqrt(time_gap)
        state_0_mean = (state - (drift * time_gap)) / root_gap
        return self.diffusion_dist.d_log_density(state_0_mean) / root_gap
...
class LorenzPrior(pydpf.Module):
    def __init__(self, state_dim: int, device: Union[str, torch.device] = "cpu", generator: torch.Generator = torch.default_generator, initial_range=None):
        super().__init__()
        self.ones_not_zero = torch.zeros((1,1,state_dim), device=device, requires_grad=False)
        self.ones_not_zero[:,:,1:] = 1
        self.device = device
    ...
    def d_log_density(self, state:Tensor, time):
        return -(state-self.ones_not_zero)**2